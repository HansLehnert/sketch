...
int main(int argc, char* argv[]) {
    ...
    unsigned short* sign_seeds[N_HASH];
    seeds_values = new unsigned short[N_HASH * settings.max_length * 4];
    for (unsigned int i = 0; i < N_HASH; i++) {
        seeds[i] = &seeds_values[settings.max_length * 2 * (2 * i)];
        sign_seeds[i] = &seeds_values[settings.max_length * 2 * (2 * i + 1)];
        for (unsigned int j = 0; j < settings.max_length * 2; j++) {
            seeds[i][j] = rand() & ~(~0 << HASH_BITS);
            sign_seeds[i][j] = rand() & ~(~0 << HASH_BITS);
        }
    }
    ...
    for (int n = 0; n < settings.n_length; n++) {
        ...
        // Create sketch
        int sketch[N_HASH][1 << HASH_BITS] = {0};
        ...
        for (unsigned int i = 0; i < data_vectors.size(); i++) {
            ...
            for (unsigned int j = 0; j < N_HASH; j++) {
                unsigned short sign_hash;
                unsigned short hash;

                hash = hashH3(data_vectors[i], seeds[j], length * 2);
                sign_hash = hashH3(data_vectors[i], sign_seeds[j], length * 2);
                ...
                hits[j] = (sign_hash & 1) ? sketch[j][hash] : -sketch[j][hash];
            }

            // Median calculation
            // Won't work as expected for an even value of N_HASH
            std::nth_element(hits, hits + N_HASH / 2, hits + N_HASH);

            // Heavy hitter detection
            if (hits[N_HASH / 2] >= settings.threshold[n]) {
                for (int k = 0; k < N_HASH; k++) {
                    std::clog << hits[k] << ", ";
                }
                std::clog << std::endl;

                heavy_hitters[n][data_vectors[i]] = hits[N_HASH / 2];
            }
        }
